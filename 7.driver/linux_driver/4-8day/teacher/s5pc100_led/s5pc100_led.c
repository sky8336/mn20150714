...
static ssize_t s5pc100_led_write(struct file *file, const char __user *buf, size_t count, loff_t *loff)
{
	...
	if(count != sizeof(struct led_action))   //传送的数据大小为结构体大小
		return -EINVAL;
                  //(void *)&action 该函数此处参数要求是void*类型 int*也可以，但是action结构体不可以
...
static int s5pc100_led_init(void)      //模块加载函数：参数为void，返回值为int
{
	int ret;
...
static void s5pc100_led_exit(void)   //模块卸载函数：参数为void，返回值为void
{
	dev_t devno = MKDEV(led_major, led_minor);
